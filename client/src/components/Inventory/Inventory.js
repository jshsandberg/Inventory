...
const Modal = (props) => {
	const { updateInventory, item } = props;
			...
			<div className="modal-dialog">
				<div className="modal-content">
					...
					<div className="modal-footer">
						...
						<button
							onClick={() => updateInventory(input, item.i)}
							type="button"
...
function InventoryItems() {
	...
	const beforeMount = async () => {
		...
		try {
			...
			const testInventory = await API.getInventory("5fdd8e6166b5690004150a34")
	...
	const updateInventory = (newItem) => {
		API.updateInventory(newItem._id, newItem).then((res) => console.log(res));
	};
	...
	return (
		<>
							...
							return (
								<>
									<tr>
										...
										{item.quantity < 5 ? (
											<td id="qty-col" style={{ backgroundColor: "#ff000052" }}>
												{item.quantity}
											</td>
										) : (
											<td id="qty-col">{item.quantity}</td>
										)}
										<td>
											<button
												key={i}
			...
			<Shipment shipment={inventoryState}></Shipment>
		</>
	);
}
